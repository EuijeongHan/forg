...
[단일판매·공급계약체결]  ※ 실사용자가 지목한 4가지를 우선 추출한다
- 계약금액, 최근 매출액 대비 비율(%) — 규모 판단의 핵심
- 계약기간(시작일 ~ 종료일)
- 거래상대방(계약상대)과 회사와의 관계, 체결계약명
- '기타 투자판단과 관련한 중요사항' 항목의 내용
- 정정공시면 무엇이 바뀌었는지(금액·기간 변경 등)
원문에 없는 항목은 아예 언급하지 않는다 — '정보 없음'을 나열하지 않는다.
...
def build_prompt(corp_name, report_nm, content):
    ...
    if content:
        ...
        # 원문을 못 가져온 경우. 항목별로 '정보 없음'을 나열하면 사용자는
        # 공시에 내용이 없는 줄 안다 — 실제로는 우리가 못 읽은 것이다
        # (2026-08-26 사용자 신고). 한 줄로 성격만 말하고 원문으로 보낸다.
        return nl.join([
            "기업명: " + corp_name, "공시 유형: " + report_nm, "",
            "원문을 가져오지 못했습니다. 공시 유형만 보고 이 공시가 무엇인지 한 문장으로만",
            "설명하고, 항목별 값은 추측하지 마세요. '정보 없음' 같은 목록을 만들지 말고,",
            "마지막에 원문 확인이 필요하다고 한 줄로 안내하세요.",
        ])
