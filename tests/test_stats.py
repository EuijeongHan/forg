...
async def main():
    ...
    # 2026-09-18: 클릭을 카운터로 기록하기 시작했으므로(#82) "클릭은 기록하지
    # 않습니다"는 더 이상 사실이 아니다. 지켜야 할 성질은 문구가 아니라,
    # '보낸 양'과 '실제 사용'을 섞지 않고 무엇을 안 남기는지 밝히는 것이다.
    check("발송을 참여로 오독하지 않게 경고", "공시가 많은 날 늘어납니다" in text)
    check("실제 사용은 따로 보라고 안내", "실제로 쓰는지는 '기능 사용'을 보세요" in text)
    check("기록하지 않는 것을 명시", "누가·어느 기업인지는 기록하지 않습니다" in text)
