...
async def main():
    ...
    # 이용 기록은 설계가 두 번 바뀌었다(#82 카운터 → 이벤트 로그). 문구는 따라
    # 바뀌어도, 지켜야 할 성질은 하나다: '보낸 양'과 '실제 사용'을 한 숫자로
    # 합치지 않고, 실제 사용을 어디서 보는지 가리킨다.
    check("발송을 참여로 오독하지 않게 경고", "공시가 많은 날 늘어납니다" in text)
    check("실제 사용은 따로 보라고 안내", "실제 사용은 '최근 14일'을 보세요" in text)
