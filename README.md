...
> 텔레그램으로 알림. 이름의 G는 **Grounded** — 공시 원문·정형 데이터에 근거한 것만
> 전한다는 원칙이다. (비공식 유래는 for G(eonsoo) — 기관투자자인 동생을 위해 시작한 프로젝트.)
...
모듈 구조와 DART 규약은 로컬 운영 문서 CLAUDE.md(개괄)·SKILL.md(구현 세부)에 정리돼
있다 (에이전트용 컨텍스트 문서로, 이 리포에는 포함되지 않는다).
...
| `/my` | 관심기업의 오늘 중요 공시 |
| `/my 유상증자` | 위 결과를 검색어로 좁힘 (이번 조회에만 적용, 저장 안 됨) |
| `/my 어제` · `/my 20260810` | 지난 날짜 조회 (검색어와 조합 가능) |
| `/market` · `/market 감사보고서` | 전체 시장의 오늘 중요 공시 (문법 동일) |
| `/ask 질문` | 자연어 공시 검색 (베타) — 과거 공시까지, 멀티턴 30분 |
| `/feedback 내용` | 오류·불편 신고 (저장 우선, 운영자 즉시 전달) |
| `/help` | 전체 사용법 |
| `/settings` | 설정 |
| `/deletedata` | 내 데이터 전체 삭제 (확인 후 즉시 삭제) |

조회 범위(관심기업 `/my` / 전체 `/market`)와 검색어는 명령에서 바로 드러난다.
저장된 필터가 결과를 조용히 바꾸지 않는다 — `/keyword`·`/mykeyword`는 이 이유로
폐기됐고, 입력하면 새 사용법을 안내한다. 구 명령 `/today`·`/mytoday`는 `/my`의
별칭으로 당분간 유지된다(조회를 수행하며 새 이름을 안내).
